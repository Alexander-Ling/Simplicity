                        ...
                        #Generating requested concentrations for each compound
                          ...
                          for(i in 1:length(temp_compounds)){
                            ...
                            for(j in 1:nrow(temp_dat)){
                              if(j == 1){
                                to_return <- seq(from = temp_dat$Lower_Conc_Limit_uM[j], to = temp_dat$Upper_Conc_Limit_uM[j], length.out = temp_dat$n_Concentrations[j])
                              } else {
                                temp_conc <- seq(from = temp_dat$Lower_Conc_Limit_uM[j], to = temp_dat$Upper_Conc_Limit_uM[j], length.out = temp_dat$n_Concentrations[j])
